...
//  }
  ...
  def showStudents():Unit = {
    ...
    val responseFuture: Future[HttpResponse] = Http().singleRequest(HttpRequest(uri = "http://localhost:8080/student"))

    responseFuture.map {
      case HttpResponse(StatusCodes.OK, headers, entity, _) => {
        val content: Future[List[Student]] = Unmarshal(entity).to[List[Student]]
        content.onComplete {
